...
class EvaluatorBackend:
    """Mesin untuk memproses data test dan menjalankan inferensi pada semua model."""
    def __init__(self):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.test_data = [] # List of dict: {'label': str, 'sequence': np.array}
        self.classes = []
        self.results = {} # Menyimpan y_true dan y_pred per model
    ...
    def load_test_data(self):
        ...
        self.classes = sorted(test_df['label'].unique().tolist())
    ...
    def run_evaluations(self):
        """Menjalankan test untuk ketiga model secara berurutan."""
        self.results = {
            'faiss': {'y_true': [], 'y_pred': []},
            'lstm': {'y_true': [], 'y_pred': []},
            'transformer': {'y_true': [], 'y_pred': []}
        }
        ...
        try:
            ...
            for item in self.test_data:
                ...
                std_seq = fm.interpolate_sequence(seq, fm.FAISS_TARGET_FRAMES).astype('float32')
...
class EvalUI:
    def __init__(self, root):
        ...
        self.btn_run = tk.Button(top_frame, text="Mulai Evaluasi Semua Model", bg="#0d6efd", fg="white", font=("Arial", 11, "bold"), command=self.run_evaluation)
    ...
    def run_evaluation(self):
        ...
        def task():
            ...
            if not status:
                ...
                self.root.after(0, lambda: self.btn_run.config(state="normal", text="Mulai Evaluasi Semua Model"))
                return
                
            self.backend.run_evaluations()
    ...
    def render_comparison(self):
        ...
        ax.set_title("Perbandingan Performa 3 Arsitektur (Macro Average)", fontweight="bold")
        ax.set_ylim(0, 1.05)
        for p in ax.patches:
    ...
    def setup_detail_tab(self):
        ...
        # Area Visual (Confusion Matrix di kiri, Tabel di Kanan)
