...
def record_manual_dynamic(vocab_name, split_type, mp_device="CPU"):
    """Merekam gerakan secara manual dengan durasi bebas."""
    # Atur Backend MediaPipe secara dinamis
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while True:
            ...
            cv2.putText(frame, 'Tekan "S" Mulai/Stop. "Q" Batal.', (10,55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1)
    ...
    if len(raw_sequence) < 5: return False, "Gerakan terlalu pendek."

    if vocab_name == 'idle': trimmed = raw_sequence
    else: trimmed = di.auto_trim_sequence(raw_sequence, movement_scores)

    if len(trimmed) < 5: return False, "Gerakan terlalu pendek setelah di-trim."

    # Format ID baru: {split}_{manual}_{nama_file}
    # Karena ini rekaman langsung (bukan file), kita kasih nama dummy_rekaman_uuid.avi
    video_id = f"{split_type}_manual_rekaman_{uuid.uuid4().hex[:6]}.avi"
    df_rows = []
...
class AppUI:
    ...
    def setup_ui(self):
        ...
        # Pilihan MediaPipe Backend
        frame_mp = tk.Frame(frame_kanan)
        frame_mp.pack(pady=(0,10))
        tk.Label(frame_mp, text="MediaPipe Backend:", font=("Arial", 10, "bold")).pack(side="left")
    ...
    def btn_import_click(self):
        ...
        if is_direct:
            ...
            tk.Button(btn_frame, text="Batal", command=deselect_all, bg="#e2e3e5").pack(side="left", expand=True, fill="x", padx=2)
            
            def confirm():
                for f, var in check_vars.items():
                    if var.get(): final_selection.append(os.path.join(parent_folder, f))
                win.destroy()
        ...
        status, msg = di.bulk_import(final_selection, split_type, mp_device)
    ...
    def btn_rekam_click(self):
        ...
        mp_device = self.combo_mp_device.get()
        status, msg = record_manual_dynamic(self.selected_vocab, split_type, mp_device)
        messagebox.showinfo("Status", msg)
        self.refresh_ui()
        self.listbox.event_generate("<<ListboxSelect>>") 

    def btn_generate_click(self):
        """Menampilkan popup checkbox untuk memilih split mana yang mau di-augmentasi"""
        win = tk.Toplevel(self.root)
    ...
    def btn_seg_click(self):
        messagebox.showinfo("Info", "Training Segmenter berjalan. Cek terminal.")
