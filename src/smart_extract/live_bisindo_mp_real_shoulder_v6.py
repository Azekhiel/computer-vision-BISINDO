#!/usr/bin/env python3
"""
live_bisindo_mp_real_shoulder_v6.py
================================
...
class LatestFrameCamera:
    ...
    def _open(self) -> cv2.VideoCapture:
        if self.use_gstreamer:
            ...
            print("[WARN] GStreamer camera open failed, fallback CAP_V4L2")
    ...
    def start(self, require_frame: bool = False, timeout: float = 2.0) -> "LatestFrameCamera":
        self.running = True
        self.thread = threading.Thread(target=self._loop, daemon=True)
        self.thread.start()
        t0 = time.perf_counter()
        wait_timeout = max(0.0, float(timeout))
        while self.frame is None and time.perf_counter() - t0 < wait_timeout:
            time.sleep(0.01)
        if require_frame and self.frame is None:
            self.release(join_timeout=3.0)
            raise RuntimeError(f"Camera src={self.src} opened but produced no frames within {wait_timeout:.1f}s.")
        return self
    ...
    def release(self, join_timeout: float = 3.0) -> dict[str, float | bool]:
        ...
        self.cap = None
        self.last_release_info = {
            "camera_released": bool(released),
            "camera_thread_alive_after_release": alive_after,
            "camera_release_ms": float((time.perf_counter() - t0) * 1000.0),
        }
        return dict(self.last_release_info)
...
class UltraMediaPipeExtractor:
    def __init__(
        ...
        shoulder_smooth_alpha: float = 0.35,
    ) -> None:
        ...
        self.shoulder_smooth_alpha = float(shoulder_smooth_alpha)
        self.shoulder_real_seen = False
    ...
    def _process_pose(self, frame_bgr: np.ndarray) -> None:
        ...
        if self.frame_i == 1 or self.frame_i % self.pose_every == 0 or self.shoulder_age > self.pose_every * 4:
            ...
            if np.isfinite(sh[:, :2]).all():
                # Real shoulder anchor. Smooth it a bit so normalization does not jitter,
                # but keep it responsive enough to follow the signer.
                if self.shoulder_real_seen and np.isfinite(self.shoulders[:, :2]).all():
                    a = float(np.clip(self.shoulder_smooth_alpha, 0.0, 1.0))
                    mixed = self.shoulders.copy()
                    mixed[:, :3] = (a * sh[:, :3] + (1.0 - a) * self.shoulders[:, :3]).astype(np.float32)
                    mixed[:, 3] = sh[:, 3]
                    self.shoulders = mixed
                else:
                    self.shoulders = sh
                    self.shoulder_real_seen = True
                self.shoulder_age = 0
            else:
                self.shoulder_age += 1
        else:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--pose-proc-width", type=int, default=192, help="Pose processing width if mp-pose is enabled.")
    p.add_argument("--pose-every", type=int, default=5, help="Run pose every N frames. 1=most real shoulder, 5=balanced.")
    ...
    p.add_argument("--shoulder-smooth-alpha", type=float, default=0.35, help="Higher = shoulders follow faster; lower = smoother shoulder anchor.")
...
def main() -> None:
    ...
    print("=" * 72)
    print("BISINDO MediaPipe V6 Real Shoulder")
