...
def make_best_extract_args(**overrides) -> argparse.Namespace:
    """Public helper used by import/migration/UI code."""
    return make_best_args(**overrides)
...
def extract_video_arrays(
    ...
    args = args or make_best_extract_args(quiet=True, no_gif=True)
    ...
    return {
        ...
        "feature_version": FEATURE_SCHEMA if args.feature_mode == FEATURE_MODE and arr.shape[1] == FEATURE_DIM else f"smart_v8_{args.feature_mode}_{arr.shape[1]}",
        "extract_profile": EXTRACT_PROFILE if args.smart_mode == "best" else f"smart_v8_{args.smart_mode}",
...
def process_one_video(video_path: Path, args: argparse.Namespace, out_root: Optional[Path] = None) -> Optional[Path]:
    result = extract_video_arrays(video_path, args=args, include_frames=True)
    if result is None:
        return None

    arr = result["features"]
    meta = result["frames"]
    overlay_frames = result["overlay_frames"]
    skeleton_frames = result["skeleton_frames"]
    target_fps = float(result["target_fps"])
    ...
    np.savez_compressed(
        ...
        extract_profile=result["extract_profile"],
    )
